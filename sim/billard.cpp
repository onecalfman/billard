...
void Billard::RunTaste0() {
    Mode = 0;
}

void Billard::RunTaste1() {
    Mode = 1;
}
...
void Billard::CheckHoles() {
    ...
    if(kugeln[0].pos[1] == 800.0)    {
        kugeln[0].v=TVektor(0.0,0.0);
        kugeln[0].pos = TVektor(-635.0,0.0);
    }
    K0[0] = kugeln[0].pos[0];
...
void Billard::HandleBallCollision() {
    ...
    double delta, dist;
    for(int k = 0; k < N; k++) {
        kugeln[k].next = kugeln[k].pos + kugeln[k].v * (1 - my);
    }
    for(int i = 0; i < N; i++) {
